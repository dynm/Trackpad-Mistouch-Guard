...
enum AppIconGenerator {
    ...
    static func writePNG(to url: URL, pixelSize: Int) throws {
        let image = makeImage(size: CGFloat(pixelSize))
        guard
            let tiff = image.tiffRepresentation,
            let bitmap = NSBitmapImageRep(data: tiff),
            let pngData = bitmap.representation(using: .png, properties: [:])
        else {
            throw NSError(domain: "AppIconGenerator", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to encode PNG"])
        }
    ...
    private static func drawIcon(in rect: CGRect) {
        ...
        highlight.shadowColor = NSColor(calibratedWhite: 1, alpha: 0.22)
        highlight.shadowBlurRadius = rect.width * 0.03
        highlight.shadowOffset = NSSize(width: 0, height: -rect.width * 0.012)
...
guard arguments.count == 3 else {
    fputs("usage: swift generate_app_icon.swift <output-path> <pixel-size>\n", stderr)
    exit(2)
}

let outputURL = URL(fileURLWithPath: arguments[1])
let pixelSize = Int(arguments[2]) ?? 0
guard pixelSize > 0 else {
    fputs("pixel-size must be positive\n", stderr)
    exit(2)
}
...
try AppIconGenerator.writePNG(to: outputURL, pixelSize: pixelSize)
